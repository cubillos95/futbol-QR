...
//datos encuentros terminados
router.post('/EncuentrosResusmen',(req,res)=>{
    const {campeonato} = req.body
    play.play.tablaConclucion(campeonato).then(x => res.send(x))
...
//Cuando se comete alguna accion : GOL ,ROJA ,AMARRILLA
router.post('/Encuentros', (req, res) => {
    session = req.session;
    if (session.userid) {
        const { campeonato } = req.body
        play.play.encuentros(campeonato, req.body).then(x => res.send(x))
    }
    else {
        res.render('formulario')
    }
})
//sacar datos de la tabla accion
router.post('/EncuentrosDato', (req, res) => {
    session = req.session;
    if (session.userid) {
        const { campeonato, codigo } = req.body
        play.play.datosEncuentros(campeonato, codigo).then(x => res.send(x))
    }
    else {
        res.render('formulario')
    }
})
//una vez terminado el partido se sube ,los datos del encuentro
router.post('/conclucion', (req, res) => {
    session = req.session;
    var hoy = new Date();
    if (session.userid) {
        const { campeonato, dato } = req.body
        const fecha = hoy.getFullYear() + ',' + hoy.getMonth() + ',' + hoy.getDate()
        const responsable = session.nombre + ' ' + session.apellido
        play.play.conclucion(campeonato,dato,fecha,responsable).then(x => res.send(x))
