...
const adminCampeonatos = {
    ...
    //*************************************************************** */
    //consulta user
    consultaUser: () => new Promise((resolve) => {
        MongoClient.connect(url, (err, db) => {
            if (err) throw err
            var dbo = db.db('user');
            dbo.collection('user').find({}).toArray((err, resul) => {
                if (err) {
                    db.close()
                    resolve('error')
                }
    ...
    //*************************************************************** */
    //consultar elementos de un usuario
    consulElemeUser: (user) => new Promise((resolve) => {
        MongoClient.connect(url, (err, db) => {
            if (err) throw err
            var dbo = db.db('user');
            dbo.collection('user').find({ name: user }).toArray((err, resul) => {
                if (err) {
                    db.close()
                    resolve('error')
                }
    ...
    //******************************************************************** */
    //subir responsables de los campeonatos
    usuariosCampeonatos: (user, campeonato, nombre, apellido, cedula) => new Promise((resolve) => {
        MongoClient.connect(url, (err, db) => {
            if (err) throw err

            var dbo = db.db("user");
            dbo.collection('userCamp').insertOne({ user, campeonato, nombre, apellido, cedula }, (err, resul) => {
                if (err) {
                    db.close()
                    resolve('Error')
                }
                db.close()
                resolve('Usuario agregado a un campeonato')
            })
        })
    }),
    //********************************************************************* */
    //lista de jugadores 
    listJugadores: (campeonato, equipo) => new Promise((resolve) => {
        MongoClient.connect(url, (err, db) => {
            if (err) throw err
            var dbo = db.db(campeonato);
            dbo.collection(equipo).find({}).toArray((err, resul) => {
                if (err) {
                    db.close()
                    resolve('error')
                }
